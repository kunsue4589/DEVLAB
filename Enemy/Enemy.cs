...
public class Enemy : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        inSightRange = Physics.CheckSphere(transform.position, sightRange,whatIsoPlayer);//check SightRange
        inAttackRange = Physics.CheckSphere(transform.position, attackRange, whatIsoPlayer);//check attackRange
    ...
    void Patroling()
    {
        if (!walkPointSet) SerchWalkPoint();// set new way point
        
        if(walkPointSet) 
        {
        agent.SetDestination(walkPoint);//walk to way point
        }
    ...
    void ChasePlayer()
    {
        agent.SetDestination(playerTransform.position);//walk to player
      
    }
    void AttackPlayer() 
    {
        animator.SetTrigger("attack"); //set animation attack
        ...
        if(!alreadyAttacked)
        {
            ...
            Invoke(nameof(ResetAttack),timeBetweenAttack);//wait for next attack
